...
class DatabaseSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $tasks = public_path('sqls/tasks.sql');
        DB::unprepared(file_get_contents($tasks));
        $this->command->info('Role_User table tasks!');

        $task_user = public_path('sqls/task_user.sql');
        DB::unprepared(file_get_contents($task_user));
        $this->command->info('Role_User table task_user!');
